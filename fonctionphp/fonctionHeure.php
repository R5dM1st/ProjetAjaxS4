...
function insertDateJournéeClassique($medecin, $date){
    ...
    if ($conn) {
        try {
            // Formatage de la date pour correspondre au format attendu par la base de données
            ...
            for ($i = 8; $i < 18; $i++) {
                ...
                $stmt->bindParam(':date_dispo', $formatted_date, PDO::PARAM_STR); // Utilisation de la date formatée
                $stmt->bindParam(':heure_dispo', $heure, PDO::PARAM_STR);
                $count = $stmt->fetchColumn();
                if ($count == 0) {
                    // Utilisation de la date formatée dans les requêtes SQL suivantes également
...
function insertDateJournéeSpecial($medecin, $date, $heure_min, $heure_max){
    ...
    if ($conn) {
        try {
            ...
            if ($stmt->rowCount() > 0) {
                echo "<h3>Les heures de la journée ont bien été ajoutées.</h3>";
                header("refresh:1;url=home_log_medecin.php");
            } else {
                echo "<h3>Erreur lors de l'ajout des heures de la journée.</h3>";
            }
        } catch (PDOException $e) {
            echo 'Error : ' . $e->getMessage();
        }
